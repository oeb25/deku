...
function normalize (children) {
  // This keeps track of the node index not including
  // falsy nodes to make DOM references easier.
  var nodeIndex = 0;
  ...
  return children.map(function (node) {
    ...
    // Empty nodes are kept in so the indexes are maintained
    if (node == null) {
      return node
    }

    // Strings/Numbers are treated as text nodes
...
function ElementNode (tagName, attributes, key, children) {
  this.type = 'element'
  this.attributes = attributes ? parseAttributes(attributes) : {}
